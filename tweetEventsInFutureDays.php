...
foreach(array('site_url','twitter_app_key','twitter_app_secret','twitter_user_key','twitter_user_secret','days') as $var) {
    if (!isset($config[$var]) || !$config[$var]) {
        die("Missing config variable: ".$var."\n");
    }
}
if (intval($config['days']) < 1) {
...
$dayToCheck = new DateTime("",new DateTimeZone("UTC"));
$dayToCheck->add(new \DateInterval("P".$config['days']."D"));
$dayToCheck->setTime(0,0,0);
$dayToCheckStart = $dayToCheck->getTimestamp();
$dayToCheck->setTime(23,59,59);
$dayToCheckEnds = $dayToCheck->getTimestamp();
$events = array();
foreach($data->data as $event) {
    if (!$event->deleted && !$event->cancelled && $event->start->timestamp >= $dayToCheckStart && $event->start->timestamp <= $dayToCheckEnds) {
        $events[] = $event;
    }
}
